...
const toggleIncidentMode = async (req, res) => {
    try {
        const result = await adminService.toggleIncidentMode(req.user.id, req.body.activate, req.body.reason, req.user.ward_id);
...
const getIncidentMode = async (req, res) => {
    try { res.json(await adminService.getIncidentMode(req.user.ward_id)); } catch (err) { res.status(500).json({ error: err.message }); }
};
...
const getUsers = async (req, res) => {
    try { res.json(await adminService.getAllUsers(req.query.role, req.user.ward_id)); } catch (err) { res.status(500).json({ error: err.message }); }
};
